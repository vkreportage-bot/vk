      ...
      <div className="mx-auto grid min-h-dvh w-full max-w-[1600px] lg:grid-cols-[240px_minmax(0,1fr)]">
        <aside className="hidden min-h-dvh border-r border-black/10 px-5 py-6 lg:flex lg:flex-col">
          <div className="mb-10">
            <Link
              href="/admin"
              className="inline-block text-[26px] font-black leading-none tracking-[-0.09em]"
            >
              VK
            </Link>
            <p className="mt-3 text-[10px] font-semibold uppercase tracking-[0.2em] text-black/35">
              Administration
            </p>
          </div>
          ...
          <div className="mt-8 space-y-2 border-t border-black/10 pt-5">
            <Link
              ...
              className="flex min-h-11 items-center gap-3 rounded-xl bg-white px-3 text-sm font-medium shadow-[0_1px_0_rgba(0,0,0,0.05)] ring-1 ring-black/10 transition hover:bg-black hover:text-white"
            >
              <Plus size={17} /> Nouveau projet
            </Link>
            <Link
              href="/admin/blog/new"
              className="flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-medium text-black/55 transition hover:bg-black/[0.045] hover:text-black"
            >
          ...
          <div className="mt-auto space-y-1 border-t border-black/10 pt-5">
            <Link
              href="/"
              target="_blank"
              className="flex min-h-10 items-center gap-3 rounded-xl px-3 text-sm text-black/45 transition hover:bg-black/[0.045] hover:text-black"
            >
            ...
            <form action="/api/admin/logout" method="post">
              <button
                type="submit"
                className="flex min-h-10 w-full items-center gap-3 rounded-xl px-3 text-sm text-black/45 transition hover:bg-black/[0.045] hover:text-black"
              >
        ...
        <div className="min-w-0 pb-24 lg:pb-0">
          <header className="flex h-16 items-center justify-between border-b border-black/10 px-5 md:px-8 lg:h-[76px] lg:px-10">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-black/35 lg:hidden">
                VK / Admin
              </p>
            ...
            <div className="flex items-center gap-2">
              {unreadCount > 0 ? (
                <Link
                  ...
                  className="inline-flex min-h-9 items-center gap-2 rounded-full border border-black/10 bg-white px-3 text-xs font-medium"
                >
              ...
              <Link
                ...
                className="hidden size-9 items-center justify-center rounded-full border border-black/10 bg-white text-black/60 sm:inline-flex lg:hidden"
              >
              ...
              <form action="/api/admin/logout" method="post" className="lg:hidden">
                <button
                  ...
                  className="inline-flex size-9 items-center justify-center rounded-full border border-black/10 bg-white text-black/60"
                >
                  <LogOut size={15} />
                </button>
              </form>
          ...
          {navigation.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "relative flex min-h-14 flex-col items-center justify-center gap-1 rounded-xl text-[8px] font-medium transition sm:text-[9px]",
                  active ? "bg-black text-white" : "text-black/45"
                )}
              >
                <span className="relative">
                  ...
                  {showUnread ? (
                    <span
                      className={cn(
                        "absolute -right-3 -top-2 inline-flex min-w-5 items-center justify-center rounded-full px-1 py-0.5 text-[9px] font-bold",
                        active ? "bg-white text-black" : "bg-black text-white"
                      )}
                    >
